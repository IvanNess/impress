...
const updateServiceTypes = (state, {type, payload})=>{
    ...
    if(state===undefined){
        return[
            ...
            {
                name: 'Видеомонтаж(+ базовая анимация)',
                stringCode: 'edit',
                auxTypes: ['play', 'combine'],
                error: false,
                // auxPanel: {
            ...
            {
                name: 'Локации для съемки(аренда студии, офиса, квартиры)',
                stringCode: 'location',
                auxTypes: ['play', 'combine'],
                open: 'hide',
                error: false,
    ...
    switch(type){ 
        ...
        case 'CALC_APP_CLICKED':
            ...
            if(!payload || !payload.isInput){
                ...
                console.log('newServiceTypes', newServiceTypes)
            }
        ...
        case 'SHOW_CALC_APP':
            newTypes = [...state.calc.serviceTypes]
                return updateNewTypesWithFocused(newTypes, state)
